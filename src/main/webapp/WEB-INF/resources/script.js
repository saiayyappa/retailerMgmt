...
function search(rid) {
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			var retailDetail = document.getElementById('retailDetail');
			r = JSON.parse(this.responseText);
			var myTable = "<table class='table table-hover'><tr class='table-primary'><th>Name</th><td>"
					+ r.name + "</td></tr>";
			myTable += "<tr><th>State</th><td>" + r.state + "</td></tr>";
			myTable += "<tr><th>Country</th><td>" + r.country + "</td></tr>";
			myTable += "<tr><th>pincode</th><td>" + r.pincode + "</td></tr>";
			myTable += "<tr><th>Mobile</th><td>" + r.mobile + "</td></tr>";
			myTable += "<tr><th>email</th><td>" + r.email + "</td></tr>";
			myTable += "</table>";
...
function paginate(pageNo, column, order) {
	var xhttp = new XMLHttpRequest();
	var url;
	url = "http://localhost:8082/paginate?page=" + pageNo + "&size=5";
	if (column == 0 && order == 0) {
		url = "http://localhost:8082/sortByNameAsc?page=" + pageNo + "&size=5";
	} else if (column == 1 && order == 0) {
		url = "http://localhost:8082/sortByAddressAsc?page=" + pageNo
				+ "&size=5";
	} else if (column == 2 && order == 0) {
		url = "http://localhost:8082/sortByEmailAsc?page=" + pageNo + "&size=5";
	} else if (column == 3 && order == 0) {
		url = "http://localhost:8082/sortByMobileAsc?page=" + pageNo
				+ "&size=5";
	} else if (column == 0 && order == 1) {
		url = "http://localhost:8082/sortByNameDesc?page=" + pageNo + "&size=5";
	} else if (column == 1 && order == 1) {
		url = "http://localhost:8082/sortByAddressDesc?page=" + pageNo
				+ "&size=5";
	} else if (column == 2 && order == 1) {
		url = "http://localhost:8082/sortByEmailDesc?page=" + pageNo
				+ "&size=5";
	} else if (column == 3 && order == 1) {
		url = "http://localhost:8082/sortByMobileDesc?page=" + pageNo
				+ "&size=5";
	}
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			for (var i = 0; i < list.length; i++) {
				var address = list[i].state + ' ' + list[i].country + ' '
						+ list[i].pincode;
				myTable += "<tr><td>" + list[i].name + "</td><td>" + address
						+ "</td><td>" + list[i].email + "</td><td>"
						+ list[i].mobile + "</td></tr>";
			}
			myTable += "</table>";
			...
			if (r.totalElements == 0) {
				...
				var retailDetail = document.getElementById('retailDetail');
				var retailDetail = document.getElementById('paginate');
				paginate.innerHTML = "";
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			for ( var key in counter) {
				myTable1 += "<tr><td>" + key + "</td><td>" + counter[key]
						+ "</td></tr>";
			}
			myTable1 += "</table>";
